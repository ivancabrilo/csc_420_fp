...
def parse_args():
    ...
    p.add_argument("--out", type=str,
                   default="../results/logs/res152_4gpu.txt")
    return p.parse_args()
...
def main():
    ...
    print(f"ResNet152 4-GPU pipeline: global_batch={args.global_batch}, "
    ...
    # 4-stage model
    s1 = ResNet152Stage1_4().to(dev0)
    s2 = ResNet152Stage2_4().to(dev1)
    s3 = ResNet152Stage3_4().to(dev2)
    s4 = ResNet152Stage4_4().to(dev3)
    ...
    # append so each (m, global_batch) pair is logged
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for x, y in train_loader:
                ...
                for xb, yb in zip(x_chunks, y_chunks):
                    ...
                    h1 = s1(xb0)                 # GPU 0
                    h2 = s2(h1.to(dev1))         # GPU 1
                    h3 = s3(h2.to(dev2))         # GPU 2
                    out = s4(h3.to(dev3))        # GPU 3
